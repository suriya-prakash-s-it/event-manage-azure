    ...
    useEffect(() => {
        const fetchEvent = async () => {
            try {
                ...
                // Check if the loaded event actually belongs to the organizer
                const eventOrganizerId = event.organizer._id || event.organizer;
                const currentUserId = user._id || user.id;

                if (eventOrganizerId !== currentUserId && user.role !== 'Admin') {
                    navigate('/dashboard');
                    return;
                }
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="pt-4 flex justify-end space-x-4 border-t border-gray-100 mt-8">
                        ...
                        <button 
                            ...
                            disabled={saving}
                        >
                            {saving ? <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-white"></div> : 'Save Changes'}
                        </button>
                    </div>
                </form>
            </div>
